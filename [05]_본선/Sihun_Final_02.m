...
%             moveback(droneObj, 'distance', 0.2);
            if quad_up_cnt < 1
                quad_up_cnt = quad_up_cnt + 1;
                moveup(droneObj, 'distance', 0.2);
                continue;
            elseif quad_right_cnt < 1
                quad_right_cnt = quad_right_cnt + 1;
                moveright(droneObj, 'distance', 0.2);
                continue;
            elseif quad_down_cnt < 2
                quad_down_cnt = quad_down_cnt + 1;
                movedown(droneObj, 'distance', 0.2);
                continue;
            elseif quad_left_cnt < 2
                quad_left_cnt = quad_left_cnt + 1;
                moveleft(droneObj, 'distance', 0.2);
                continue;
            else
                moveup(droneObj, 'distance', 0.4);
